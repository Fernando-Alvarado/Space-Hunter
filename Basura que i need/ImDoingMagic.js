...
class balas{
    ...
    constructor(PrincipalMat, whereX, whereY, whereZ, ObjX, ObjY, ObjZ, Killed, limiteCampoJuego){ //kileed se refiere a la nave a
        ...
        function Adding3D(array, funct, dist, limit){
            for (let index = 0; index < array[0].length; index++) {
             var DistanciaEjeZ = (funct(array[0][0], array[1][0], array[0][index], array[1][index])/dist)
                array[2][index] = Math.round(limit- DistanciaEjeZ);

            }
            return array;
        }
        this.MatBalas =  Adding3D(graficadora(this.WherX, this.WherY, this.ObjX, this.ObjY, this.limCampJue), this.INeedTonKnow,this.DistEntreKillernKilled, 10);
